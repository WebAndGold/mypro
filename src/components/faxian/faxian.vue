						...
						<router-link to="/linshi" v-for="(item,index) in gwlist" :key="index">
							<li class="example-listitem">
								<div class="listitem listitem1 listitem-gw">
									...
									<div class="item-sum">{{item.sum}}人阅读</div>
								</div>
	...
	export default{
		...
		data(){
			return {
				...
				images:['http://127.0.0.1/my-xingmuImg/faxian-banner1.png','http://127.0.0.1/my-xingmuImg/faxian-banner2.png','http://127.0.0.1/my-xingmuImg/faxian-banner3.png'],
				list: [],
		      	allLoaded: false,
		     	topStatus: '',
		      	topAllLoaded: false,
			...
			loadTop(id){
				setTimeout(()=>{
					if(this.topAllLoaded == false){
						...
						axios({
							method:'post',
							url:'http://127.0.0.1/allPHP/phpbases/daxianloadmore.php',
							data:pnoData,
							responseType:'json'
						}).then((response)=>{
		...
		created(){
			...
			axios({
				moehod:'get',
				url:'http://127.0.0.1/allPHP/phpbases/faxian_gw.php',
				responseType:'json',
			}).then((response)=>{
...
//					console.log(this.gwlist)
			...
			axios({
				moehod:'get',
				url:'http://127.0.0.1/allPHP/phpbases/faxian_hwd.php',
				responseType:'json',
			}).then((response)=>{
//				console.log(response.data)
				if(response.data.length!=0){
...
//					console.log(this.hwdlist)
			...
			axios({
				moehod:'get',
				url:'http://127.0.0.1/allPHP/phpbases/faxian_hkd.php',
				responseType:'json',
			}).then((response)=>{
//				console.log(response.data)
				if(response.data.length!=0){
